...
const MusicToggle: React.FC = () => {
  const { musicAvailable, musicEnabled, setMusicEnabled } = useMusic();
  ...
  return (
    <button
      ...
      className="flex items-center gap-2 min-h-[3rem] px-4 py-2 rounded-xl bg-zinc-800 hover:bg-zinc-700 text-lg font-semibold text-white focus:outline-none focus:ring-4 focus:ring-netflix-red/60"
      aria-pressed={musicEnabled}
      aria-label={musicEnabled ? 'Turn calm music off' : 'Turn calm music on'}
    >
      {musicEnabled ? (
        <>
